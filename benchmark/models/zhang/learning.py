...
def train(model, X, Y, A, optimizer, alpha=1):
    ...
    adv_opt.apply_gradients(zip(dULa, adv_vars))

    dWLp = clas_tape.gradient(clas_loss, clas_vars)  # regular grads for classifier

    dWLa = clas_tape.gradient(
        adv_loss, clas_vars
    )  # grads for W with the adversarial loss
    ...
    clf_opt.apply_gradients(zip(clas_grads, clas_vars))
...
def train_loop(model, raw_data, train_dataset, epochs, opt=None):
    ...
    for epoch in range(epochs):
        ...
        print(
            "> {} | {} | {} | {} | {}".format(
                epoch + 1, clas_loss, adv_loss, clas_acc, adv_acc
            )
        )
